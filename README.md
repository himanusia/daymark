...
3. Create an **Android OAuth client** for package `com.himanusia.daymark` and the SHA-1 certificate used by the build. `cd android && ./gradlew signingReport` prints the debug certificate.
...
The display name is **H- Countdown**. `daymark` remains only as the current package/repository slug until a final product name is chosen.
